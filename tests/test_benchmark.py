...
from cvrp_benchmarker import Benchmarker, CvrpProblemDescription, Runner, HyperParameter, TrustedRunResult, cost_history_entry
...
def test_studies_init():
    ...
    with TemporaryDirectory() as tempdir:
        ...
        assert isinstance(bench.studies[0][1], Study)
        assert bench.studies[1][1] is None
...
def test_parallelization_capabilities():
    ...
    with TemporaryDirectory() as tempdir:
        ...
        bench.tune_parameters(n_trials=2, n_jobs=2, concurrency=2)
        end_bench = time.time()
        assert bench.studies[0][1].best_trial is not None
        assert bench.studies[1][1].best_trial is not None
        assert bench.studies[2][1].best_trial is not None
...
def test_benchmarking():
    ...
    with TemporaryDirectory() as tempdir:
        ...
        bench.benchmark(n_runs=4, n_jobs=4)